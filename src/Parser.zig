    ...
    if (init_d.d.func_declarator != null and init_d.initializer.node == .none and init_d.d.ty.isFunc()) fn_def: {
        ...
        if (init_d.d.old_style_func != null) {
            ...
            param_loop: while (true) {
                ...
                while (true) {
                    ...
                    for (init_d.d.ty.params()) |*param| {
                        ...
                        try p.errStr(.parameter_missing, d.name, p.tokSlice(d.name));
                    }
                    ...
                    try p.syms.syms.append(p.gpa, .{
                        .kind = .def,
                        .name = p.tokSlice(d.name), //todo fixme
                        .tok = d.name,
                        .ty = d.ty,
                        .val = .{},
                    });
            ...
            for (init_d.d.ty.params()) |param| {
                ...
                try p.syms.syms.append(p.gpa, .{
                    .kind = .def,
                    .name = p.comp.getString(param.name, p.pp.tokens.items(.loc)), // todo interned string
                    .tok = param.name_tok,
                    .ty = param.ty,
                    .val = .{},
                });
    ...
    const l_brace = p.eatToken(.l_brace) orelse {
        ...
        if (try p.syms.findTag(p, p.tok_ids[kind_tok], ident, p.tok_ids[p.tok_i])) |prev| {
            ...
            try p.syms.syms.append(p.gpa, .{
                ...
                .name = p.tokSlice(ident), //TODO FIXME record_ty.name,
                .tok = ident,
                .ty = ty,
                .val = .{},
            });
        ...
        while (true) {
            if (p.eatToken(.l_bracket)) |l_bracket| {
                ...
                if (!cur_ty.isRecord()) {
                    ...
                    try p.errStr(.no_such_field_designator, period, p.tokSlice(field_tok));
                    return error.ParsingFailed;
                }
