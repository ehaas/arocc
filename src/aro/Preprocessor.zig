...
fn expandMacroExhaustive(
    ...
    while (do_rescan) {
        ...
        while (idx < moving_end_idx) {
            ...
            const macro_hideset = pp.hideset.get(.{ .id = macro_tok.loc.id, .byte_offset = macro_tok.loc.byte_offset });
            if (pp.hideset.contains(macro_hideset, expanded)) {
                idx += 1;
                continue;
            }
            ...
            macro_handler: {
                if (macro.is_func) {
                    ...
                    const r_paren_hideset = pp.hideset.get(.{ .id = r_paren.loc.id, .byte_offset = r_paren.loc.byte_offset });
                    var hs = try pp.hideset.intersection(macro_hideset, r_paren_hideset);
                    ...
                    for (res.items) |*tok| {
                        ...
                        const tok_hideset = pp.hideset.get(.{ .id = tok.loc.id, .byte_offset = tok.loc.byte_offset });
                        const new_hideset = try pp.hideset.@"union"(tok_hideset, hs);
                        try pp.hideset.put(.{ .id = tok.loc.id, .byte_offset = tok.loc.byte_offset }, new_hideset);
                    }
                    ...
                    const hs = try pp.hideset.prepend(.{ .id = macro_tok.loc.id, .byte_offset = macro_tok.loc.byte_offset }, macro_hideset);
